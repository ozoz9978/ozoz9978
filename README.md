## Hello 👋 MinGeun Jo
* 그동안 학습한 내용 & 프로젝트 진행사항을 github에 정리합니다

### ⚡ 기술역량 : 
Programming Language : Python 기본코드 작성 및 함수화 가능, 문제 발생시 공식 문서와 자료를 참고하여 해결
Framework / Library 
Pandas, Numpy : 데이터 프레임 생성 및 조작, 데이터 필터링, 그룹화, 정렬 및 집계, 결측치 처리, 데이터 정제 & 전처리
Plotly, Seaborn, Matplotlib : 
Scikit-learn : 기계 학습 모델 생성, 훈련 및 평가, 하이퍼 파라미터 튜닝, 통계 모델링 및 가설검정
Tensorflow, Keras : 딥러닝 모델 gh repo clone ozoz9978/DeepLearning
Basic Statistics
Dashboard : Lookerstudio, Powerbi


### ⭐ history
1. SCFI 지수를 이용한 쥬요 항로별 해상운임(Ocean Freight) 예측 모델 개발
2. 항로별 해상운임 예측모델을 활용한 종합 물류 서비스 '청해지니' 개발
3. 텍스트 마이닝으로 제약회사 마케팅 전략 제안

